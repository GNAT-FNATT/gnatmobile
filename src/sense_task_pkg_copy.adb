...
package body Sense_Task_Pkg is
   ...
   front: UltrasonicSensor := (EchoPin => MB_P1,
      ...
      sensors: UltrasonicSensors:=(left, front);
   ...
   protected body UltrasonicHandler is
      ...
      begin
        ...
        -- Put_Line("Interrupt");
         ...
         --       if LatchRegisterPort0.Arr(sensors(sensorIndex).EchoPin.Pin) = Latched then
   ...
   begin
      ...
      for index in sensors'Range loop
         ...
         sensors(index).EchoPin.Configure_IO(NoPullConfiguration);
      ...
      loop
         ...
         for sensorIndex in sensors'Range loop
           ...
           -- delay 0.001;
            Pool: loop
               exit Pool when sensors(sensorIndex).EchoStopTrigger = True;
               --Put_Line(sensorIndex'Image);  
               if sensors(sensorIndex).EchoStartTrigger = False and sensors(sensorIndex).EchoPin.Set = True then
                  sensors(sensorIndex).EchoStartTrigger := True;
                  sensors(sensorIndex).RisingTimestamp := CLock;
                  --Put_Line("Rising");
               elsif sensors(sensorIndex).EchoStopTrigger = False and sensors(sensorIndex).EchoPin.Set = False then
                  sensors(sensorIndex).EchoStopTrigger := True;
                  sensors(sensorIndex).FallingTimestamp:= Clock;
                  --  Put_Line("Falling");
            ...
            --  while(not(sensors(sensorIndex).EchoStopTrigger = True or Clock - triggerTime > ( Milliseconds(30))))loop
            --      Asm("nop");
            --  end loop;
         ...
         delay until startTime + Milliseconds(1000);
      end loop;
         end sense;
end Sense_Task_Pkg;
